...
function scr_player_move_y(y_change){
	with(obj_player){
		...
		if((grid_visible_y + y_change > obj_manager.grid_vertical_size - obj_manager.inner_boarder && obj_manager.grid_y_start + obj_manager.grid_vertical_size < obj_manager.level_height) || (grid_visible_y + y_change < obj_manager.inner_boarder &&  obj_manager.grid_y_start > 0)) {	//otherwise, if the next cell is past the inner grid border and the grid hasn't reached the bottom of the level
